...
from configs.final_student_runs import FINAL_STUDENT_RUN_ORDER, FINAL_STUDENT_RUNS  # noqa: E402

RUNS = FINAL_STUDENT_RUNS
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Run full-project full-student teacher-KD + hard-negative training on our fixed split."
    )
...
def build_env(retrieval_distillation_dir: Path) -> dict[str, str]:
    env = os.environ.copy()
    existing = env.get("PYTHONPATH", "")
    prefix = str(retrieval_distillation_dir)
...
def selected_run_keys(run_key: str) -> list[str]:
    return FINAL_STUDENT_RUN_ORDER if run_key == "all" else [run_key]
...
def training_command(args: argparse.Namespace, paths: dict[str, Path], image_root: Path, run_key: str) -> list[str]:
    ...
    output_dir = (
        paths["work_root"]
        / "runs_final_students_l4"
        / f"{config.final_run_name}_s1{args.stage1_epochs}_s2{args.stage2_epochs}"
    )
    cmd = [
        sys.executable,
        str(paths["retrieval_distillation_dir"] / "train_full_student_kd_hn.py"),
        "--artifacts-dir",
...
def main() -> None:
    ...
    env = build_env(paths["retrieval_distillation_dir"])
