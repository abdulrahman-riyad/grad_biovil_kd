...
from run_hard_negative_l4 import HARDWARE_PROFILES, RUNS, command_to_string
...
def eval_dir(work_root: Path, run_key: str) -> Path:
    return work_root / "eval_hard_negative_l4" / run_key


def log_path(work_root: Path, run_key: str) -> Path:
    stamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    path = work_root / "logs_single_l4" / f"{run_key}_{stamp}.log"
...
def main() -> None:
    ...
    train_cmd = [
        sys.executable,
        str(project_root / "project_repo" / "ops" / "gcp_l4" / "run_hard_negative_l4.py"),
        "--run-key",
        args.run_key,
        "--epochs",
        str(args.epochs),
    ...
    eval_cmd = [
        sys.executable,
        str(project_root / "project_repo" / "ops" / "gcp_l4" / "evaluate_l4.py"),
        "--run-key",
        args.run_key,
        "--epochs",
        str(args.epochs),
