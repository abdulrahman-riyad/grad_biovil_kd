...
from train_stage2_hard_negatives_l4 import (
    HARDWARE_PROFILES,
    RUNS,
    RunConfig,
    command_to_string,
...
def eval_output_dir(work_root: Path, key: str, pool: int | None, seed: int | None) -> Path:
    if pool is None:
        return work_root / "eval_retrieval_l4" / key / "pool_full"
    return work_root / "eval_retrieval_l4" / key / f"pool_{pool}_seed_{seed}"
...
def student_eval_command(
    ...
    cmd = [
        sys.executable,
        str(paths["retrieval_distillation_dir"] / "evaluate_student_retrieval.py"),
        "--checkpoint",
        str(checkpoint),
        "--output-dir",
        str(output_dir),
...
def teacher_eval_command(
    ...
    cmd = [
        sys.executable,
        str(paths["retrieval_distillation_dir"] / "evaluate_teacher_retrieval.py"),
        "--artifacts-dir",
        str(paths["artifacts_dir"]),
        "--splits-dir",
        str(paths["splits_dir"]),
...
def main() -> None:
    ...
    env["PYTHONPATH"] = f"{paths['retrieval_distillation_dir']}{os.pathsep}{env.get('PYTHONPATH', '')}"
    ...
    if not args.dry_run:
        summarize(paths["work_root"] / "eval_retrieval_l4")
