...
|---|---|---|
| `distil_biobert` | `nlpie/distil-biobert` | Matches the teammates' DistilBioBERT text-student baseline. |
...
Run `week3/503-lightweight-text-encoder-comparison.ipynb` after uploading the updated `week3/track_ab` folder to Kaggle. Compare the new pilot results against the six rows above before selecting any lightweight text student for a full run.
